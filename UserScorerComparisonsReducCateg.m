function UserScorerComparisonsReducCateg(numbUsers)
filename1 = 'C:\Sleepdata\Results\Christine_results_04302008\247B\TR_Meg247Bcsc11_2Mod.xls';
filename2 = 'C:\Sleepdata\Results\Christine_results_04302008\247B\TR_247B_ES.xls';
filename3 = '';
...
name=char(answer(1,:));
fid = fopen(['C:\Sleepdata\Results\Christine_results_04302008\' name 'Reduc.xls'],'w');
...
fid = fopen(['C:\Sleepdata\Results\Christine_results_04302008\' name 'Reduc.xls'],'a');
agree1 = 0;



t=1;
numberMismatch = zeros(3,6);
percentMismatch = zeros(3,6);
for i = 1:3
    switch i
        case 1
            Index = find((user{1}(:,3) == 1)|(user{1}(:,3) == 4));
            s1 = 1; s2 = 4;
        case 2
            Index = find((user{1}(:,3) == 2)|(user{1}(:,3) == 6));
            s1 = 2; s2 = 6;
        case 3
            Index = find(user{1}(:,3) == 3);
            s1 = 3; s2 = 0;
    end
    agree = 0;
    ...
    if isempty(userState) == 0
        ...
        for k = 1:p
            if (user2State(k,1) == s1 | user2State(k,1) == s2)
                agree = agree + 1;
                agree1 = agree1 + 1;
            else
