function autoScorerUserMatrix2sEpochs
numbUsers=1;
% AS_File = 'C:\Sleepdata\Results\Rat954BaselineHcPt1_11282012_Gina_AS.xls';
% filename1 = 'C:\SleepData\Results\Rat954BaselineCtxPt1_11282012_GinaPoeASConvertedToManualFormat.xls';
% filename2 = '';
% filename3 = '';
% filename4 = '';
...
fid = fopen(['C:\Sleepdata\' name 'ASvUserMatrix.xls'],'w');
fprintf(fid,'Auto-Scorer v User');
fprintf(fid,'\n');
fclose(fid);

%for j = 1:numbUsers
    fid = fopen(['C:\Sleepdata\' name 'ASvUserMatrix.xls'],'a');
    agree = 0;
    for k = 1:n
        if user(k,1) == AS(k,1)
            agree = agree + 1;
    ...
    for i = 1:6
        user1Totals(i) = sum(numberMismatch(i,:)); % User
        user2Totals(i) = sum(numberMismatch(:,i)); % Auto-Scorer
    ...
    F = 'bob'; %eval(strcat('filename', num2str(j)));
    fprintf(fid,F);
    fprintf(fid,'\n');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% for j = 1:numbUsers
%     fid = fopen(['C:\Sleepdata\' name 'ASvUserTrackingMatrix.xls'],'a');
%     agree = 0;
%     for k = 1:n
